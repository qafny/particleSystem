...
let analyze_one_circuit (str_input : string) (err : float) (t :  float) (flag_path : int) : (int * int * int * int * int * int * float * int * int) =
    ...
    if flag_path = 0 then
      ...
      (nq1, nqm, nq1_bf, nqm_bf, tc, npau, r)

	else if flag_path > 0 && flag_path < 10  
      then let (cc, cir_bf, tc, r, npau) = lowprog_to_circ ~verbose:true lp nqbit err t flag_path in
      let (nq1, nqm) = summarize_counts cc nqbit r in
      let (nq1_bf, nqm_bf) = summarize_counts cir_bf nqbit r in
      (nq1, nqm, nq1_bf, nqm_bf, tc, npau, r)	
    ...
    in (nqbit, List.length lp, nq1, nqm, nq1_bf, nqm_bf, tc, npau, r)
...
let run (err : float) (t : float) (path : string) (flag_path : int) : (int * int * int * int * int * int * float * int * int) =
...
let () =
  ...
  let (nqubit, nterm, single_qubit_gates, multi_qubit_gates, single_qubit_gates_bfopt, multi_qubit_gates_bfopt, tc, npau, r) = run !err !t path !path_flag in
  let json_data =
    `Assoc
