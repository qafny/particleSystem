#!/usr/bin/env python3
"""
Benchmark the vendored Tetris baseline (thirdparty/phoenix) on QBlue datasets.

This uses the Tetris implementation integrated in Phoenix's repo (the `tetris.*`
Python package under thirdparty/phoenix) and emits QBlue-comparable IBM-basis
...
def tetris_compile_to_circuit(
    ph: ParsedHamiltonian,
    *,
    time_t: float,
    swap_coefficient: int,
    k: int,
    reverse_labels: bool,
    ...
    qc, metrics = synthesis_lookahead(
        ...
        graph=graph,
        use_bridge=False,
        swap_coefficient=int(swap_coefficient),
        k=int(k),
    )
    metrics = dict(metrics)
    metrics["reverse_labels"] = reverse_labels
    return qc, metrics
...
def main() -> int:
    ...
    ap.add_argument("--out", type=Path, default=Path("results/tetris.csv"))
    ...
    ap.add_argument("--swap-coefficient", type=int, default=3, help="Tetris routing swap cost coefficient.")
    ap.add_argument("--k", type=int, default=10, help="Tetris lookahead window.")
            ...
            for tval in args.ts:
                ...
                try:
                    ...
                    qc, metrics = tetris_compile_to_circuit(
                        ph,
                        time_t=tval,
                        swap_coefficient=args.swap_coefficient,
                        k=args.k,
                        reverse_labels=args.reverse_labels,
                    )
                ...
                w.writerow(
                    {
                        ...
                        "tetris_swap_count": metrics.get("tetris_swap_count", ""),
                        "tetris_cx_count": metrics.get("tetris_cx_count", ""),
                        "tetris_bridge_count": metrics.get("tetris_bridge_count", ""),
                        "IR_total": metrics.get("IR_total", ""),
                        "IR_remain": metrics.get("IR_remain", ""),
                        "IR_cancel_ratio": metrics.get("IR_cancel_ratio", ""),
                        "error": err_s,
                    }
                )
                csvf.flush()
