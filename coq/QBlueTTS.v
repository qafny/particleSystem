...
Definition get_nseg (t : R) (lp: norm_prog) : nat := 
  let sum_coef := (fold_left (fun acc x => acc + Rabs(((fst x)))) lp 0)%R in
...
Definition findK (err t : R) (lp : norm_prog) := 
...
(* (H1 + H2 + ...)^k *)
Fixpoint cal_lowp_power (nqubit k : nat) (lp : norm_prog) : norm_prog := 
  match k with
  | 0 => [(R1, fun x => paulii)]
...
Fixpoint taylor_exp (t : R) (nqubit k : nat) (lp : norm_prog) : norm_prog := 
  match k with 
...
Fixpoint cntl2pauli_helper (nl : list nat) : norm_prog :=
  match nl with
...
Fixpoint LCU_digital_ibm (nqubit : nat) (vi : norm_prog) : ucom ExtractionGateSet.U :=
  match vi with
  | [] => SKIP
  | (amp, f) :: app => useq (synth_digital_ibm_apauli nqubit (amp) f) (LCU_digital_ibm nqubit app)
  end.

(* Build each V with control bit: I+∣011⟩⟨011∣⊗(V−I):
idx: index of V; nqcn: # of control qubits; nqv: # of qubits in Vi; lp: list of Vi *)
Definition build_cntlV (lp : norm_prog) (nqcn nqv idx: nat) : ucom ExtractionGateSet.U :=
  match nth_error lp idx with
  | None => SKIP
  | Some vi => 
    let nl := cnt2bin idx nqcn in
    let control := cntl2pauli_helper nl in
    let t2 := plus_ten_plus nqcn nqv control (vi :: (-R1, fun _ => paulii) :: nil) in
